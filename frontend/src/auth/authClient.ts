...
export function getAuthConfig(): AuthConfig {
  return {
    domain: normalizeAuthDomain(getEnvValue('VITE_AUTH0_DOMAIN')),
    clientId: getEnvValue('VITE_AUTH0_CLIENT_ID'),
    audience: getEnvValue('VITE_AUTH0_AUDIENCE'),
    redirectUri: getEnvValue('VITE_AUTH0_REDIRECT_URI') || `${window.location.origin}/callback`,
    scope: getEnvValue('VITE_AUTH0_SCOPE') || 'openid profile email offline_access',
  };
}

export function isAuthConfigured(): boolean {
  const { domain, clientId, audience, redirectUri } = getAuthConfig();
  return Boolean(domain && clientId && audience && redirectUri);
}
...
function readRequestState(): {
  ...
  try {
    ...
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { state?: string; nonce?: string; verifier?: string };
...
function persistRequestState(state: string, nonce: string, verifier: string): void {
...
export async function loginWithRedirect(): Promise<void> {
  ...
  if (!isAuthConfigured()) {
    throw new Error('Auth0 is not configured. Set VITE_AUTH0_* environment variables.');
  }
  ...
  persistRequestState(state, nonce, verifier);

  const authorizeUrl = new URL(`${config.domain}/authorize`);
...
export async function handleAuthCallback(): Promise<AuthSession | null> {
  ...
  if (!requestState) {
    throw new Error('Auth0 callback state is missing. Please log in again.');
  }

  if (state && requestState.state !== state) {
    throw new Error('Auth0 callback state does not match. Please log in again.');
  }

  const config = getAuthConfig();
  const tokenResponse = await fetch(`${config.domain}/oauth/token`, {
    method: 'POST',
    headers: {
  ...
  if (!tokenResponse.ok) {
    ...
    throw new Error(detail || 'Failed to exchange the Auth0 authorization code.');
  }
  ...
  if (
    ...
    throw new Error('The Auth0 access token is not valid for the Atlas backend audience.');
  }
  ...
  const session: AuthSession = {
    ...
    user: {
      sub: String(claims.sub ?? 'unknown-user'),
